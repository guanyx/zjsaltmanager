...
function PopUp(id, config){
	...
	this.mm = "max";
	this.imgMin = "frame/alarm/min.gif";
	this.imgMax = "frame/alarm/max.gif";
} 
	...
	create : function (){
		...
		t.innerHTML = "<a id='closeButton' href='#'></a>"+
			  "<a id='switchButton' href='#'></a>"+
			   "<iframe id='"+this.id+"_content_iframe' src="+c.contentUrl+" frameborder=0 scrolling=no allowtransparency=true width='100%' height='100%' style='height:100%'></iframe>"; 
		var sBtn = this.switchButton = $_t(t,'a',"switchButton");
		sBtn.style.cssText='position:absolute;z-index:2;font-size:0px;line-height:0px;left:220px;top:6px;width:15px;height:15px;background-image:url("frame/alarm/min.gif");';
		$addEL(sBtn,"click",$dele(this,"switchMode"),true);
		$addEL(sBtn,"click",$cancelEvent,true); 
		var btn = $_t(t,'a',"closeButton");
		btn.style.cssText='position:absolute;z-index:2;font-size:0px;line-height:0px;left:240px;top:6px;width:15px;height:15px;background-image:url("frame/alarm/close.gif");';
...
*/
	var cfg={
		...
		display    : true,
		//content url
		contentUrl   : "frame/alarm/alarmcontent.aspx",
	...
	var cookieName="zj_salt_popup_next_display_time";
	/**
...
function doit(){ 
	if(document.readyState == 'loaded' || document.readyState == 'complete'){ 
		window.setTimeout(job,6000);// delay 3s
	}else{
		window.setTimeout(doit,3000);// delay 3s
		return;
	}
} 
