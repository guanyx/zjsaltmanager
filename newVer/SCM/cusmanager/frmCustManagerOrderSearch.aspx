<%@ Page Language="C#" AutoEventWireup="true" CodeFile="frmCustManagerOrderSearch.aspx.cs" Inherits="SCM_frmCustManagerOrder" %>
...
var iscancel = getParamerValue('cancel'); 
if(iscancel=='true')
    window.document.title="客户历史订单";
else
    window.document.title="客户历史取消订单";
</script>
<script type="text/javascript">
...
Ext.onReady(function() {
    ...
    function openViewOrderDtlWin() {
        ...
        document.getElementById("editIFrame").src = "frmCustManagerOrderEdit.aspx?OpenType=query&id=" + selectData.data.OrderId;
    }
           
    function QueryDataGrid() { 
        OrderMstGridData.baseParams.OrderId=orderPanel.getValue();
        OrderMstGridData.baseParams.CustomerNo=codePanel.getValue();
        OrderMstGridData.baseParams.CustomerName=namePanel.getValue();		       
        if(iscancel=='true')
            OrderMstGridData.baseParams.isActive = 0;
        else
   ...
   var orderPanel = new Ext.form.NumberField({
    ...
    //客户编号
   var codePanel = new Ext.form.TextField({
        style: 'margin-left:0px',
        cls: 'key',
        xtype: 'textfield',
        fieldLabel: '客户编号',
...
,		{
        ...
        uploadOrderWindow.addListener("hide", function() {
           document.getElementById("editIFrame").src = "frmCustManagerOrderEdit.aspx?OpenType=query&id=0";//清楚其内容，建议在子页面提供一个方法来调用
        });
        ...
        ({
            url: 'frmCustManagerOrderSearch.aspx?method=getOrderList',
