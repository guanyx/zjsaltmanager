...
var dateDocument = window.endDateLayer.document;
if(dateDocument == null || dateDocument==undefined)
    dateDocument=window.frames.endDateLayer.contentDocument;
dateDocument.writeln(strFrame);
dateDocument.close();		//解决ie进度条不结束的问题
...
var odatelayer=dateDocument.all;		//存放日历对象
...
function setday(tt,obj) //主调函数
{
	...
	var offSrc = th.offsetParent;
	var offY = th.offsetTop;
	var offSetDivTop=0;

	while(offSrc.tagName!="BODY")
	{
		offY = offY + offSrc.offsetTop;
		//取日期控件所在parent的top偏移量
		if(offSrc.offsetParent.tagName=="BODY")
		{
			offSetDivTop=offSrc.offsetTop;
		}
		
		offSrc = offSrc.offsetParent;
	}
	if(offY>211+th.offsetTop+50)
	{
		dads.top =offY - 211-th.clientHeight+15;
	}
	else
	{
		dads.top =offY+th.clientHeight+20;
	}
	
	
//	dads.top = (ttyp=="image") ? ttop+thei : ttop+thei+6;
	dads.left = tleft;
	//08-6-1日修改，outObject固定为调用控件，obj就设置为是否开启时间
//	outObject = (arguments.length == 1) ? th : obj;
	...
	var dates = outObject.value.split(' ');
	var times;
	if(dates.length>1)
	{
	    times = dates[1].split(':');
	...
	if(r!=null){
		...
		if(d.getFullYear()==r[1] && d.getMonth()==r[2] && d.getDate()==r[3])
		{
			...
			if(times)
	        {
	            outDate = new Date(r[1],r[2],r[3],times[0],times[1],0);
			    parent.meizzTheHour = outDate.getHours();
			    parent.meizzTheMinute = outDate.getMinutes();
			    meizzTheHour = parent.meizzTheHour;
			    meizzTheMinute = parent.meizzTheMinute;
			}
		}
		else
		{
	...
	//{
		//bUseTime=true;
		bUseTime = (arguments.length == 1) ?true:obj;
		bImgSwitch();
		odatelayer.bUseTimeLayer.innerHTML=bImg;
...
//本项目的时间一般精确到分就可以了，所以在初始时只要设置秒为0即可
var meizzTheSecond = 0;
...
bind(document, "onkeyup", function()		//按Esc键关闭，切换焦点关闭
{
...
function evaSetTime()		//设置用户选择的小时、分钟
{
	...
	{
	    var date = outDate.getDate();
	    WriteDateTo(meizzTheYear,meizzTheMonth,date,meizzTheHour,meizzTheMinute,meizzTheSecond);
	}
