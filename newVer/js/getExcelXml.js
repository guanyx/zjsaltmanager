	...
	if(title ==undefined) title=this.title;
        ...
        return '<?xml version=\"1.0\" encoding=\"gbk\"?>' +
            ...
            '<ss:Styles>' +
                ...
                '</ss:Style>' +'<ss:Style ss:Parent=\"even\" ss:ID=\"evendate\">' +'<ss:NumberFormat ss:Format=\"yyyy&quot;年&quot;m&quot;月&quot;d&quot;日&quot;;@\" />' +//[ENG][$-409]dd\-mmm\-yyyy;@
                ...
                '</ss:Style>' +'<ss:Style ss:Parent=\"odd\" ss:ID=\"odddate\">' +'<ss:NumberFormat ss:Format=\"yyyy&quot;年&quot;m&quot;月&quot;d&quot;日&quot;;@\" />' +//[ENG][$-409]dd\-mmm\-yyyy;@
...
createWorksheet: function(title,includeHidden) {
 // Calculate cell data types and extra class names which affect formatting
        ...
        for (var i = 0; i < cm.getColumnCount(); i++) {
            ...
            if (includeHidden || !cm.isHidden(i) || cm.getDataIndex(i)=="ClassName") {
 ...
 // Generate the data rows from the data in the Store
        for (var i = 0, it = this.store.data.items, l = it.length; i < l; i++) {
            ...
            for (var j = 0; j < cm.getColumnCount(); j++) {
                if (includeHidden || !cm.isHidden(j) ||cm.getDataIndex(j)=="ClassName" ) {
                    if (cm.getDataIndex(j) != "")
                    {
                        var v = r[cm.getDataIndex(j)];
                        if(v != "" && cellType[k] != 'DateTime')
                        {
				if(typeof cm.config[j].renderer == 'function')
                            	v=cm.config[j].renderer(v);
                        }
                        if (cellType[k] !== "None") {

                         t += '<ss:Cell ss:StyleID=\"' + cellClass + cellTypeClass[k] + '\"><ss:Data ss:Type=\"' + cellType[k] + '\"><![CDATA[';                         
