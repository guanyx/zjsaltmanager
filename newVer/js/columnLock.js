...
Ext.ux.grid.LockingGridView = Ext.extend(Ext.grid.GridView, {
	...
	updateAllColumnWidths : function(){
		...
		this.lockedBody.dom.style.width = lw;
		...
		for(i = 0, len = ns.length; i < len; i++){
			...
			if(row.firstChild){
				...
				trow = row.firstChild.rows[0];
				for (j = llen; j < clen; j++) {
				   trow.childNodes[j].style.width = ws[j - llen];
				}
			}
		}
	...
	layout : function(){
		...
		if(g.autoHeight){
			...
			if(this.innerHd){
				this.lockedInnerHd.style.width = lw + 'px';
				this.innerHd.style.width = (vw - lw) + 'px';
			}
		}
		if(this.forceFit){
		...
		this.mainWrap.dom.style.left = this.getLockedWidth();
		this.onLayout(vw, vh);
	},
