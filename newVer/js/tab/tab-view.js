...
************************************************************************************************************/
	...
	var tabView_maxNumberOfTabs = 16;	// Maximum number of tabs
	...
	function showTab(parentId,tabIndex)
	{
		...
		if(activeTabIndex[parentId]>=0){
			...
			img.src = '../images/tab/tab_right_inactive.gif';
		...
		img.src = '../images/tab/tab_right_active.gif';
	...
	function rolloverTab()
	{
		if(this.className.indexOf('tabInactive')>=0){
			...
			img.src = '../images/tab/tab_right_over.gif';
		}
	...
	function initTabs(mainContainerID,tabTitles,activeTab,width,height,closeButtonArray,additionalTab)
	{
		...
		for(var no=0;no<tabTitles.length;no++){
			...
			span.style.color='#4C75B4';
			span.style.fontWeight='bold';
			aTab.appendChild(span);

			if(closeButtonArray[no]){
				var closeButton = document.createElement('IMG');
				closeButton.src = '../images/tab/close.gif';
	 ...
	 * else create
	 */
	function createNewTab2NoAjax(mainContainerID,tabTitle,tabContent,tabContentUrl,closeButton)
	{
	    var tabs = dhtmlgoodies_tabObj[mainContainerID].getElementsByTagName('SPAN');
	    var isfind = 0 ;
		for(var no=0;no<tabs.length;no++)
		{
		    if(tabs[no].innerHTML.indexOf(tabTitle + '')>-1)
		    {
		        var activeDiv = tabs[no].parentNode;
                var idArray = activeDiv.id.split('_');
                showTab(activeDiv.parentNode.parentNode.id,idArray[idArray.length-1].replace(/[^0-9]/gi,''));
                isfind = 1;
                break;
            }
		}
		if(isfind ==0)
		{
		    createNewTabNoAjax(mainContainerID,tabTitle,tabContent,tabContentUrl,closeButton)
		}
	 ...
	 * use iframe jianggl
	 */
	function createNewTabNoAjax(parentId,tabTitle,tabContent,tabContentUrl,closeButton)
	{
		if(tabView_countTabs[parentId]>=tabView_maxNumberOfTabs)return;	// Maximum number of tabs reached - return
		var div = document.createElement('DIV');
		div.className = 'dhtmlgoodies_aTab';
		...
		if(tabContentUrl){
		    div.innerHTML="<iframe src='"+tabContentUrl+"' name='"+tabTitle+"'  id='contentFrme' width='100%' marginwidth='0' height='100%'' style='display:block' scrolling='no' frameborder='0' framebordercolor='#FF0000'></iframe>";
		}
