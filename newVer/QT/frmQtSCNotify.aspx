...
<body>
   ...
   var panel = new Ext.Panel({
       heigth: 50,
       html: '<div style="width:100%;text-align:center"> <font size="4"><H1 id=checkid>浙江省盐业集团质检单</H1></font></div>'
   });
       ...
       Ext.Ajax.request({
           url: "frmQtQualityCheckNotify.aspx?method=secondcheck"
                   , params: {
                      ...
                      , checkrange: Ext.fly('quotarank').dom.value
                   },
           success: function(response, options) {
               Ext.Msg.alert("提示", "保存成功！");
               checkWindow.hide();
       ...
       Ext.Ajax.request({
           url: "frmQtQualityCheckNotify.aspx?method=getStardDetail"
                       , params: {
                           ...
                           prno: record.ProductNo
                       },
           success: function(response, options) {
            if (response.responseText.length < 5) {
               Ext.Msg.alert("提示","该产品还未配置质检模板！");
               //formulaWindow.hide();
            } else {
                formulaWindow.show();              
                ...
                Ext.getCmp("checktype").setValue(record.DicsName);
                Ext.getCmp("shipno").setValue(record.ShipNo);
                Ext.getCmp("checkno").setValue(record.DeliveryNo);
                Ext.getCmp("local").setValue(record.WhpName);
                Ext.getCmp("pname").setValue(record.ProductName);
                Ext.getCmp("pno").setValue(record.ProductNo);
                Ext.getCmp("rml").setValue(record.RepresentAmount);
                //成功显示界面
                decodeArr(record.CheckId, record.ProductNo, record.CheckType);
                //更新标题
                updateCheckOrg(record.ProductNo,record.ProductName,'质检单');
               }
           }
        ,
           failure: function() {
   ...
   function updateCheckOrg(ProductNo,ProductName,Suffix){
       var url = "frmQtSCNotify.aspx?method=getCheckOrgName";
       Ext.Ajax.request({
        url: url
       , params: {
           ProductNo:ProductNo,
       ...
       success: function(response, options) {
           ...
           if (response != null && response.responseText != 'undefined' && response.responseText != null && response.responseText != '') {               
               var data = Ext.util.JSON.decode(response.responseText);
               var title = data.CheckOrgName+Suffix;
               Ext.fly('checkid').dom.innerHTML = title;
           }
       },
       failure: function() {           
   ...
   function decodeArr(checkid, prno, checktype) {
       if (modify_prno_id == checkid) {
           ...
           var oldnodes = detail.getEl().dom.lastChild.lastChild;
           var sindex=oldnodes.childNodes.length-1; 
           if(sindex>-1)
                oldnodes.removeChild(oldnodes.childNodes[sindex]);
           oldnodes.appendChild(divobj);
           ...
           Ext.Ajax.request({
               ...
               success: function(response, options) {
                   ...
                   if (response != null && response.responseText != 'undefined' && response.responseText != null && response.responseText != '') {
                       ...
                       str += "<table border='1' cellpadding='0' cellspacing='0'style='border-collapse:collapse;background-color:#FFFFFF';border-color:#000000' width='100%'>";
                       if (data.length % 2 == 1) {
   ...
   function queryFormula() {
       ...
       customerListStore.baseParams.limit=15;
   ...
   var quotaGrid = new Ext.grid.GridPanel({
       ...
       */
       cm: new Ext.grid.ColumnModel([
		...
		{ header: '进仓日期', dataIndex: 'ProductDate' ,width:80 },
		{ header: '仓库', dataIndex: 'WhName' ,width:50 },
		{ header: '放置地点', dataIndex: 'WhpName' ,width:60 },
		{ header: '是否质检完成', dataIndex: 'CheckStatus', renderer: cktp ,width:60 }
	  ]), listeners:
	  {
