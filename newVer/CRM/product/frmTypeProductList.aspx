<%@ Page Language="C#" AutoEventWireup="true" CodeFile="frmTypeProductList.aspx.cs" Inherits="CRM_product_frmTypeProductList" %>
    ...
    Ext.onReady(function() {
        ...
        function addTypeProduct() {
            if (addTypesProductWin == null) {
                addTypesProductWin = parent.ExtJsShowWin('添加供应商品信息', 'frmTypeProductList.aspx?action=add&classId=' + classId, 'add', 800, 500);
                addTypesProductWin.show();
            }
            else {
                addTypesProductWin.show();
                parent.document.getElementById("iframeadd").contentWindow.classId = classId;
                parent.document.getElementById("iframeadd").contentWindow.loadData();
            }
        }

        function delTypePruducts() {
            ...
            Ext.Msg.confirm("系统提示", "是否正的要删除选择的商品信息！", function callBack(id) {
                ...
                if (id == "yes") {
                    Ext.Ajax.request({
                        url: 'frmTypeProductList.aspx?method=del',
                        method: 'POST',
                        params: {
                            ClassId: classId,
                            ProductIds: ids
        ...
        function saveTypePruducts() {
            ...
            Ext.Ajax.request({
                url: 'frmTypeProductList.aspx?method=save',
                method: 'POST',
                params: {
                    ClassId: classId,
                    ProductIds: ids
...
({
    url: 'frmTypeProductList.aspx?method=getTypeProductList',
...
this.selectShow = function(columnName) {
    ...
    function loadData() {
        ...
        productGridData.baseParams.ClassId = classId;
        productGridData.load({ params: { limit: defaultPageSize, start: 0} });
    }
