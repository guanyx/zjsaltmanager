...
<script type="text/javascript">
    ...
    Ext.onReady(function() {
        ...
        function printOutStock() {
                ...
                //页面提交
                Ext.Ajax.request({
                    url: 'frmAllotOutOrderList.aspx?method=outprintdate',
                    method: 'POST',
                    params: {
                        OrderId: orderIds
                    },
            ...
            function printOrderById() {
                ...
                //页面提交
                Ext.Ajax.request({
                    url: 'frmAllotOutOrderList.aspx?method=printdate',
                    method: 'POST',
                    params: {
                        OrderId: orderIds
                    },
            ...
            function AddOrderWin() {
                ...
                uploadAllotOrderWindow.setTitle("新增移库单");
                document.getElementById("editAllotOrderIFrame").src = "frmAllotOrderEdit.aspx";
            }
            ...
            function EditOrderWin() {
                ...
                //alert(selectData.data.AllotType);
                if (selectData.data.OutStatus != '0')//不是未入仓状态
                {
                    Ext.Msg.alert("提示", "只有未出仓的移库单才能编辑！");
                    return;
                }
                if (selectData.data.AllotType != '0')//跨区移库
                {
                    Ext.Msg.alert("提示", "请在跨区订单界面编辑！");
                    return;
                }
                uploadAllotOrderWindow.show();
                uploadAllotOrderWindow.setTitle("编辑移库单");
                document.getElementById("editAllotOrderIFrame").src = "frmAllotOrderEdit.aspx?id=" + selectData.data.OrderId;
            }
            ...
            function deleteOrderWin() {
                ...
                if (selectData.data.OutStatus != 0)//不是未入仓状态
                {
                    Ext.Msg.alert("提示", "已出库的移库单不能删除！");
                    return;
                }
                if (selectData.data.AllotType != 0)//跨区移库
                {
                    Ext.Msg.alert("提示", "请在跨区订单界面删除！");
                    return;
                }
                //删除前再次提醒是否真的要删除
                Ext.Msg.confirm("提示信息", "是否真的要删除选择的移库单吗？", function callBack(id) {
                    ...
                    if (id == "yes") {
                        //页面提交
                        Ext.Ajax.request({
                            url: 'frmAllotOutOrderList.aspx?method=deleteAllotOrder',
                            method: 'POST',
                            params: {
            ...
            function generateOutStockOrderWin() {
                ...
                if (selectData.data.OutStatus == 1)//已出仓状态
                {
                    Ext.Msg.alert("提示", "该移库单已经出仓！");
                    return;
                }
                //uploadOrderWindow
                uploadOrderWindow.show();
                document.getElementById("editIFrame").src = "frmInStockBill.aspx?type=W0205&id=" + selectData.data.OrderId;
                ...
                //                    document.getElementById("editIFrame").contentWindow.FromBillType='W0205';
